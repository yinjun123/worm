...
def start(num,data_queue,page_queue):
	global d,url
	while True:
		if page_queue.empty():
			break
...
def mysql(data):
	...
	try:
		...
		sql1 = "select count(*) from statements_list where phone = "+ a
		cur = mysqlcli.cursor()
		cur.execute(sql1)
		...
		if result[0] == 0:
			try:
				sql = "INSERT INTO statements_list (Booked_time,zong_money,cash,deduction,kan_tou_xi,yinghuan_money,overdue_days,order_number,name,phone,Borrowing_time,Borrowing_amount,Arrive_time,yinghuan_time,expire_yinghuan) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
				items = [data['Booked_time'],data['zong_money'],data['cash'],data['deduction'],data['kan_tou_xi'],data['yinghuan_money'],data['overdue_days'],data['order_number'], data['name'], data['phone'], data['Borrowing_time'], data['Borrowing_amount'],data['Arrive_time'], data['yinghuan_time'],data['expire_yinghuan']]
				cur = mysqlcli.cursor()
			...
			sql = "update statements_list set zong_money='%s',cash='%s',deduction='%s',kan_tou_xi='%s',yinghuan_money='%s',order_number='%s',Borrowing_time='%s',Borrowing_amount='%s',Arrive_time='%s',yinghuan_time='%s',expire_yinghuan='%s',overdue_days='%s',Booked_time='%s' where phone='%s'"%(data['zong_money'],data['cash'],data['deduction'],data['kan_tou_xi'],data['yinghuan_money'],data['order_number'],data['Borrowing_time'],data['Borrowing_amount'],data['Arrive_time'],data['yinghuan_time'],data['expire_yinghuan'],data['overdue_days'],data['Booked_time'],data['phone'])
			cur.execute(sql)
			mysqlcli.commit()
			cur.close()
			print "重复数据进行更新"
...
def main():
	...
	for crawl in range(3):
		thread = threading.Thread(target=start, args=(num, data_queue,page_queue,))
		thread.start()
